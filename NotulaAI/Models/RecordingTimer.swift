...
final class RecordingTimer: ObservableObject {
     ...
     */
    init(lengthInMinutes: Int = 0, attendees: [Meeting.Attendees] = []) {
    ...
    //TODO: Logic kayak gini lebih rapi kalau ditempatin di ModelView daripada di dalam Model
     ...
     */
    func reset(lengthInMinutes: Int, attendees: [Meeting.Attendees]) {
        self.lengthInMinutes = lengthInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension Array<Meeting.Attendees> {
    var speakers: [RecordingTimer.Speaker] {
        if isEmpty {
            return [RecordingTimer.Speaker(name: "Speaker 1", isCompleted: false)]
        } else {
            return map { RecordingTimer.Speaker(name: $0.name, isCompleted: false) }
        }
    }
}
